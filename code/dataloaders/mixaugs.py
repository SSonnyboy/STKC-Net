...
# # 1. cutmix for 2d
# # # # # # # # # # # # # # # # # # # # #
def cut_mix(unlabeled_image, unlabeled_mask):
    ...
    # get box
    u_bbx1, u_bby1, u_bbx2, u_bby2 = rand_bbox(
        unlabeled_image.size(), lam=np.random.beta(4, 4)
    )
...
def cut_mix_3d(unlabeled_image, unlabeled_mask):
    mix_unlabeled_image = unlabeled_image.clone()
    mix_unlabeled_target = unlabeled_mask.clone()
    ...
    # get box
    # img: B x C x H x W x D, lb: B x H x W x D
    u_bbx1, u_bby1, u_bbz1, u_bbx2, u_bby2, u_bbz2 = rand_bbox_3d(
        unlabeled_image.size(), lam=np.random.beta(4, 4)
    )
    ...
    for i in range(0, mix_unlabeled_image.shape[0]):
        mix_unlabeled_image[
            i, :, u_bbx1[i] : u_bbx2[i], u_bby1[i] : u_bby2[i], u_bbz1[i] : u_bbz2[i]
        ] = unlabeled_image[
            u_rand_index[i],
            :,
            u_bbx1[i] : u_bbx2[i],
